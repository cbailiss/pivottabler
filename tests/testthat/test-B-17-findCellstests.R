...
scenarios <- testScenarios("find cells tests:  max value")
for(i in 1:nrow(scenarios)) {
  evaluationMode <- scenarios$evaluationMode[i]
  processingLibrary <- scenarios$processingLibrary[i]
  description <- scenarios$description[i]
  ...
  test_that(description, {
    ...
    # pt$renderPivot()
    # paste(c(cells[[1]]$rawValue, cells[[2]]$rawValue, cells[[3]]$rawValue), collapse = '|')
    # prepStr(as.character(pt$getHtml()))
    values <- "28201|22133|8849"
    ...
    expect_equal(length(cells), 3)
    expect_equal(paste(c(cells[[1]]$rawValue, cells[[2]]$rawValue, cells[[3]]$rawValue), collapse = '|'), "28201|22133|8849")
    expect_identical(as.character(pt$getHtml()), html)
  })
}
