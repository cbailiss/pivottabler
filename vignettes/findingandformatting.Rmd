...
The `findCells()` and `getCells()` functions can be used to conditionally format a pivot table.

For example, to find the average arrival delay (in minutes) in the morning peak time, for the top 20 origin stations, broken down by the hour of the day (i.e. 5am, 6am, 7am, etc): 
...
trains <- bhmtrains %>%
  ...
  select(Status, TOC, TrainCategory, PowerType, SchedSpeedMPH, Origin, Destination,
         GbttHourOfDay, ArrivalDeltaMins, ArrivalDelayMins)

# create the pivot table
library(pivottabler)
pt <- PivotTable$new()
pt$addData(trains)
pt$addColumnDataGroups("GbttHourOfDay", fromData=FALSE, explicitListOfValues=list(5, 6, 7, 8, 9, 10))
...
It is also possible to iterate through the cells to use a continuous colour scale as opposed to three separate styles:
...
# specify some conditional formatting
