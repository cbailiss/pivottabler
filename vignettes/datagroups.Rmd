...
Note that finding total/non-total data groups/cells with the `findRowDataGroups()`, `findColumnDataGroups()` and `findCells()` functions won't work when visual totals are enabled.  See the [Finding and Formatting](findandformatting.html) vignette for more details on these functions.
...
Adding data groups one-at-a-time for complete fine-grained control will be possible in a future version.
<!-- This is  more involved and is described separately in the [Custom Layouts](customlayouts.html) vignette. -->
