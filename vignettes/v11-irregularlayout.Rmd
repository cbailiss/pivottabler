...
## Methods for Building a Data Group Hierarchy
...
- `values` specifies the filter values applied to `variableName` in order for data to match this row/column in the pivot table.
- `doNotExpand` - a logical value, default `FALSE` - specify `TRUE` to prevent the high-level methods from adding child groups.
- `isEmpty`- a logical value, default `FALSE` - specify `TRUE` to prevent any data from matching this data group.
- `isOutline` - specifies whether this data group is an outline group or not.
...
- `isTotal` - a logical value that specifies whether this data group is a total.
- `isLevelSubTotal` - a logical value that specifies whether this data group is a sub-total within a level
  + Typically there are multiple sub-totals within a level.
- `isLevelTotal` - a logical value that specifies whether this data group is the total for a level.
...
## Methods for modifying a pivot table
...
It is nonethless possible to change many properties of the data groups, however, doing so should be done with caution as this may invalidate the state of the pivot table and such code could be broken by future changes to the package.
...
The cells are tied into a hierarchy.  Moving cells is not supported (and unlikely ever to be supported).

### Removing cells

The cells are tied into a hierarchy.  Removing individual cells is not currently supported.
