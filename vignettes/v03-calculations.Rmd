...
Creating additional calculation groups is only necessary for some advanced pivot table layouts - see the [Irregular Layout](v10-irregularlayout.html) vignette for an example.
...
The data.table package can also be used - see the [Performance](v11-performance.html) vignette for details.  Note that the "count" summarise expression is specified as `.N` when using data.table, not as `n()`.  

When using data.table, be aware of some strange behaviour that leads to incorrect values in the pivot table when aggregating over columns that are also used in either row/column headings.  Again, see the [Performance](v11-performance.html) vignette for details.
...
Please see the [Performance](v11-performance.html) for more information about `pivottabler` performance and data.table.
