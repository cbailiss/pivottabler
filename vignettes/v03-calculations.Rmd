...
The data.table package can also be used - see the [Performance](v11-performance.html) for details.  Note that the "count" summarise expression is specified as ".N" when using data.table, not as "n()".  Also, when using data.table, be aware of some strange behaviour that leads to incorrect values in the pivot table when aggregating over columns that are also used in either row/column headings.  Again, see the [Performance](v11-performance.html) for details.
...
Using a custom R format function allows bespoke formatting logic to be used.  In following (contrived) example, "Mean Arr. Delay 2" includes a descriptive prefix that shows whether a number is less than, roughly equal to or greater than 3. 
