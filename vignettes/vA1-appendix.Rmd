...
The `pt$getHtml(...)`, `pt$saveHtml(...)`, `pt$renderPivot(...)`, `pt$getLatex(...)` and `pt$writeToExcelWorksheet(...)` functions all support an `exportOptions` list parameter that provides control over how NA, NaN, -Inf and Inf values in R are outout.  
