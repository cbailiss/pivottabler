...
* Example Pivot Table
* Finding Headings
* Getting Cells (based on location in the pivot table)
* Finding Cells (based on cell value inc. conditional formatting)
* Summary
* Further Reading
...
A couple of the examples in this vignette use the `mapStyling()` method to format cells based on their value.  This method is also described in the [Styling](v07-styling.html) vignette.
...
- `lowN` - an integer value specifying that only N cells containing the lowest values should be returned, e.g.
  + Specifying `lowN=1` will find the cell containing the minimum value.
  + Specifying `lowN=5` will find the five cells containing the lowest values, sorted into ascending order (lowest value first).
- `highN` - an integer value specifying that only N cells containing the highest values should be returned, e.g.
  + Specifying `highN=1` will find the cell containing the maximum value.
  + Specifying `highN=5` will find the five cells containing the highest values, sorted into descending order (highest value first).
...
There are two approaches we can take - either use `mapStyling()`, or use `findCells()` and `setStyling()`.  `mapStyling()` is described in the [Styling](v07-styling.html) vignette.
...
* The `findCells()` function is used to search for cells within the body of the pivot table matching one or more criteria.
    ...
    + The `lowN` and `highN` parameters can be used to find the cell or cells with the minimum value(s) and maximum value(s). 
