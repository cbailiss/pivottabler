...
The row/column number arguments can be specified in two different ways depending on the value of the `specifyCellsAsList` argument.  In versions v0.4.0 and earlier, the default value of the `specifyCellsAsList` argument was `FALSE`.  The default value is now `TRUE`.
...
If only one of `rowGroups` or `columnGroups` is specified, then they behave exactly the same as `groups`.  If both `rowGroups` and `columnGroups` are specified, then only cells related to both the specified row and column groups are retrieved.
