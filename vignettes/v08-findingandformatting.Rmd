...
### Finding cells by cell value

For guidance on finding cell(s) based on the value of the cell (i.e. based on the results of calculations), see the Conditional Formating section below.
...
There are two approaches we can take to this - either use `mapStyling()`, or use `findCells()` and `setStyling()`.  `mapStyling()` is described in the [Styling](v07-styling.html) vignette.
...
The examples above uses three sets of colours.  It is also possible to apply these colours as a continuous colour gradient, e.g. an average arrival delay of 3 minutes would be formatted using a mixed yellow-green colour.  Again, this can be done following the same two approaches.
