...
There are two approaches we can take to this - either use `mapStyling()`, or use `findCells()` and `setStyling()`.

`mapStyling()` is described in the [Styling](v07-styling.html) vignette.

Using `mapStyling()` with `mapType="range"` is more concise:
...
Using `findCells()` and `setStyling()`:
