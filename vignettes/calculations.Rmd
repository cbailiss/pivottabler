...
For large data frames, the current approach can significantly increase the time required to calculate the pivot table cell values.  In such cases, aggregating the data explicitly in R code (where this is possible) before creating the pivot table can reduce the overall time required.
