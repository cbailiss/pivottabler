...
- `onlyAddGroupIf` - a character value that is used as an additional filter criteria before creating each data group.  Only if some rows exist that match this filter will the data group be created.  In the image below, this allows/prevents all three of the rows being generated.
- `onlyAddOutlineChildGroupIf` - a character value that is used as additional filter criteria before creating each outline child data group.  Only if some rows exist that match this filter will the child data group be created.  In the image below, this allows/prevents just the middle row (orange "Blank") being generated.
